...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=(), eps=1e-16, prefix=""):
    ...
    px, py = np.linspace(0, 1, 23811), []  # for plotting
    ap, p, r = np.zeros((nc, tp.shape[1])), np.zeros((nc, 23811)), np.zeros((nc, 23811))
    print(conf)
    for ci, c in enumerate(unique_classes):
        ...
        interpolated_conf = np.interp(px, -conf[i], conf[i])
        df_conf = pd.DataFrame({
        'confidence': interpolated_conf,

        })
        df_conf.to_csv(Path(save_dir) / f'{prefix}_conf.csv', index=False)  # Save to CSV

        print(interpolated_conf)
...
def plot_mc_curve(px, py, save_dir=Path('mc_curve.png'), names=(), xlabel='Confidence', ylabel='Metric'):
    ...
    # 判断是不是绘制F1_curve曲线
    flag = False
    if str(save_dir).endswith('P_curve.png'):
        flag = True
        pr_dict = dict()  # lwd edit
        pr_dict['px'] = px.tolist()  # lwd edit
    # --------------------------------------------------------- #
