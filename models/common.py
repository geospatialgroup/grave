...
class FrequencyDomainFilter(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        out_channels (int): 输出特征的通道数，频域滤波器处理后希望输出的特征通道数
        """
        super(FrequencyDomainFilter, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels

        # 使用1x1卷积层（线性层）、ReLU激活函数和另一个线性层来构建可学习的频域滤波器
        self.filter_layer = nn.Sequential(
    ...
    def forward(self, x):
        ...
        返回:
        torch.Tensor: 经过频域处理（FFT、滤波、IFFT）后的输出张量，形状为 (batch_size, out_channels, height, width)
        """
        batch_size, in_channels, height, width = x.size()

        # 步骤1：将输入特征进行快速傅里叶变换（FFT）转换到频域
        x_fft = torch.fft.fftn(x, dim=(-2, -1))

        # 步骤1.1：标准化频域特征（对实部和虚部的幅度进行标准化）
        x_fft_real = x_fft.real
        x_fft_imag = x_fft.imag
        x_fft_real = (x_fft_real - x_fft_real.mean()) / (x_fft_real.std() + 1e-6)
        x_fft_imag = (x_fft_imag - x_fft_imag.mean()) / (x_fft_imag.std() + 1e-6)
        x_fft = torch.complex(x_fft_real, x_fft_imag)

        # 将频域结果的实部和虚部进行堆叠，方便后续操作
        x_fft = torch.stack((x_fft.real, x_fft.imag), dim=-1)

        # 步骤2：通过可学习的频域滤波器点乘输入的频域特征
        # 调整形状，方便与滤波器进行点乘操作
        x_fft_reshaped = x_fft.view(batch_size, in_channels, height * width, 2)
        x_fft_reshaped = x_fft_reshaped.permute(0, 1, 3, 2)  # 维度变为 (batch_size, in_channels, 2, height * width)

        # 通过可学习的频域滤波器进行处理
        filter_weights = self.filter_layer(x)
        filter_weights = filter_weights.view(batch_size, self.out_channels, 1, height * width)

        # 点乘操作，在频域上应用滤波器权重
        filtered_fft = filter_weights * x_fft_reshaped

        # 调整形状，恢复到类似原始频域特征的维度表示
        filtered_fft = filtered_fft.permute(0, 1, 3, 2).view(batch_size, self.out_channels, height, width, 2)

        # 分离实部和虚部，准备进行IFFT操作
        filtered_fft_real = filtered_fft[:, :, :, :, 0]
        filtered_fft_imag = filtered_fft[:, :, :, :, 1]

        # 构建复数形式的频域张量，用于IFFT操作
        filtered_fft_complex = torch.complex(filtered_fft_real, filtered_fft_imag)

        # 步骤3：通过快速傅里叶逆变换（IFFT）转换回到空域
        x_ifft = torch.fft.ifftn(filtered_fft_complex, dim=(-2, -1))

        # 取IFFT结果的实部作为最终输出（IFFT结果的虚部理论上应该接近0，但取实部确保数据格式符合要求）
        x_result = x_ifft.real

        return x_result
class FreqFusion_up3(nn.Module):
    def __init__(self, chn):
        super(FreqFusion_up3, self).__init__()
        self.chn = chn
        # self.device = torch.device('cuda:0')
        self.ff1 = FreqFusion(hr_channels=chn[0], lr_channels=chn[1]).cuda()
        self.ff2 = FreqFusion(hr_channels=chn[1], lr_channels=chn[2]).cuda()
        # self.ff3 = FreqFusion(hr_channels=chn[2], lr_channels=chn[3])
        self.conv1 = Conv(chn[1]+chn[2], chn[1], 1, 1).cuda()
        self.conv2 = Conv(chn[0] + chn[1], chn[0], 1, 1).cuda()
        # self.concat1 = FFM_Concat22()
        # self.conv3 = Conv(chn[0] + chn[1], chn[0], 1, 1)



    def forward(self,xs):
        # _, x3, x4_up = self.ff1(hr_feat=xs[0], lr_feat=xs[1])

        # x_cat = torch.cat([xs[0],xs[-1]],1)
        _, x2, y3_up = self.ff2(hr_feat=xs[1].cuda(), lr_feat=xs[2].cuda())
        y2 = self.conv1(torch.cat([x2.cuda(), y3_up.cuda()],1))  # channel=c
        _, x1, y2_up = self.ff1(hr_feat=xs[0].cuda(), lr_feat=y2.cuda())
        y1 = self.conv2(torch.cat([x1.cuda(), y2_up.cuda()],1))  # channel=c
...
class ELAN1(nn.Module):
    ...
    def forward_split(self, x):
        ...
        y.extend(m(y[-1]) for m in [self.cv2, self.cv3])
        return self.cv4(torch.cat(y, 1))
class HWD_ADown1(nn.Module):
    def __init__(self, c1, c2):  # ch_in, ch_out, shortcut, kernels, groups, expand
        super().__init__()
        self.c = c2 // 2
        self.cv1 = HWD(c1 // 2, self.c)
        self.cv2 = Conv(c1 // 2, self.c, 1, 1, 0)
        self.FrequencyDomainFilter = FrequencyDomainFilter(c1, c1)

    def forward(self, x):
        x0 = self.FrequencyDomainFilter(x) + x
        x = nn.functional.avg_pool2d(x0, 2, 1, 0, False, True)
        x1, x2 = x.chunk(2, 1)
        x1 = self.cv1(x1)
